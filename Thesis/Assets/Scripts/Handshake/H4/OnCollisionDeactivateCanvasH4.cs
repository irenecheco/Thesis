...
public class OnCollisionDeactivateCanvasH4 : MonoBehaviourPunCallbacks
{
            ...
            {                   
                ...
                if (!colliderPhotonView.IsMine)
                {
                    if (firstExited)
                    {
                        ...
                        firstExited = false;
                        this.transform.parent.transform.FindChildRecursive("ActivateCollider").gameObject.GetComponent<OnCollisionActivateCanvasH4>().firstEntered = true;
                    }                    
                }
            }
        }
