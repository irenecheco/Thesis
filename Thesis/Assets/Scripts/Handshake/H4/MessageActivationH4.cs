...
public class MessageActivationH4 : MonoBehaviour, IPunObservable
{
    ...
    public void ActivateMessage()
    {
        if (!photonView.IsMine && photonView != null)
        {
            ...
            rightHand.GetComponent<GrabbingH4>().SetGrabbing(this.gameObject, thisId);
            localNetRightHand.GetComponent<MessageActivationH4>().isGrabbing = true;
            localNetPlayer.GetComponent<NetworkGrabMessageActivationH4>().CallActivateGrabMessage(localId, thisId);
        }
    }
    ...
    public void DeactivateMessage()
    {
        if (!photonView.IsMine && photonView != null)
        {
            ...
            rightHand.GetComponent<GrabbingH4>().SetReleasing(this.gameObject, thisId);
            localNetRightHand.GetComponent<MessageActivationH4>().isGrabbing = false;
            this.transform.localPosition = new Vector3(0, 0, 0);
            this.transform.localRotation = new Quaternion(0, 0, 0, 0);
            this.transform.localRotation = Quaternion.Euler(0, 0, -90);
            localNetPlayer.GetComponent<NetworkGrabMessageActivationH4>().CallDeactivateGrabMessage(localId, thisId);
        }
    }
